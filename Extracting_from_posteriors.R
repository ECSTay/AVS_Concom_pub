...
for(t in 1:4){
  ...
  for(s in 1:2){
    ...
    for(r in 1:ninds){
      ...
      eta[,1] <- 0
      eta[,2] <- mu1 + (s == 2)*alpha + epsilon1 + dat$w[inds[r]]*beta1 + dat$x[inds[r]]*gamma1 + as.vector(dat$q[inds[r],]%*%t(rho1)) + as.vector(dat$c[inds[r],]%*%t(tau1)) + dat$z[inds[r]]*delta1
      if(s == 1){
        eta[,3] <- mu2 + epsilon2 + dat$w[inds[r]]*beta2 + dat$x[inds[r]]*gamma2 + as.vector(dat$q[inds[r],]%*%t(rho2)) + as.vector(dat$c[inds[r],]%*%t(tau2)) + dat$z[inds[r]]*delta2
      } else {
        eta[,3] <- - Inf
      }
...
dat_vis <- data.table(sim = rep(1:ndraws, 4*2*4),
                      ...
                      Outcome = factor(rep(rep(c("P(0 events)", "P(1 event)", "P(2 events)", "P(at least one event)"), each = ndraws), 2*4), levels = c("P(0 events)", "P(1 event)", "P(2 events)", "P(at least one event)")),

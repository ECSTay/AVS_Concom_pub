...
#extracting_posteriors <- function(draws, dat){
  ...
  dat_vis_new <- dat_vis[!Outcome == "0"]
  dat_vis_new
  
  dat_vis_new <- dat_vis_new %>%
    mutate(Outcome = case_when(Outcome == "1" ~ "P(1 event)",.default = Outcome)) %>%
    mutate(Outcome = case_when(Outcome == "2" ~ "P(2 events)",.default = Outcome))
  
  quantile(dat_vis_new$value[[1]], prob = 0.025)
  quantile(dat_vis_new$value[[1]], prob = 0.975)
  
  quantile(dat_vis_new$value[[2]], prob = 0.025)
  quantile(dat_vis_new$value[[2]], prob = 0.975)
  
  x <- dat_vis_new$value[[1]] + dat_vis_new$value[[2]]# at least one event 2mths sep
  mean(x)#at least one event sep 2 mths = 0.38
  dat_vis_new$y_mean[[9]]#0.28

  
 
  quantile1 <- vector()
  for(i in 1:ndraws){
  quantile1[i] <- quantile(dat_vis_new$value[[i]], prob = 0.025)
  }
